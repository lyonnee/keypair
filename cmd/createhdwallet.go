...
func createHdWalletAction(ctx *cli.Context) error {
	words, _ := wallet.GenerateMnemonic(mnemonicQuantity, mnemonicLang)
	seed := wallet.ToSeed(words, mnemonicPassword)

	privKey := wallet.NewPrivateKey(seed)

	fmt.Printf("\nYour new key was generated\n\n")
	fmt.Printf("Mnemonic words is:   %s\n", words)
	for i := uint32(0); i < 10; i++ {
		newPrivkey := privKey.NewChildKey(i)
		fmt.Printf("Child index %d, Public address of the key:   %s\n", i, newPrivkey.Address())
	}
